<template>
	<scroll-view class="scrollView" scroll-y refresher-background="#f1f1f1" :refresher-threshold="105"
		...
		<view class="faultView" v-for="(item,index) in deviceTypeList" :key="index" @click="goPage(item,index)">
	...
	export default {
		...
		methods: {
			...
			async getFindDeviceTypeList() {
				...
				const res = await findDeviceTypeList(data);
			...
			goPage(item, index) {
				this.$tab.navigateTo(
					`/pages/check/facilityCheck/checkInfo?itemIndex=${index}&itemList=${JSON.stringify(this.deviceTypeList)}&item=${JSON.stringify(item)}&checkInfo=${JSON.stringify(this.checkInfo)}`
				)
			}
		}
	}
