			...
			<view class="customTop" :style="'height:'+statusBarHeight+'rpx'">
				<view class="customBottom">
					...
					<view class="customTop3">
						<!-- <u-icon name="plus"  @click="submit()" size="20" bold
							color="#FF5E5E"></u-icon> -->
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
			...
			uni.$on('refreshData', (data) => {
				if (data.refreshData) {
					if (this.swiperTabCurrent == 0) {
						this.$refs.checkListAllRef.getDeviceTypeList();
		...
		methods: {
			...
			submit() {
				this.$tab.navigateTo(`/pages/check/offLine/checkInfo?checkInfo=${JSON.stringify(this.checkInfo)}`)
			}
		}
	}
</script>
