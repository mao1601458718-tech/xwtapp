...
<script>
	...
	export default {
		...
		methods: {
			goPage(item) {
				this.$tab.navigateTo("/pages/check/fhCheck/fhCheckIndex?item=" + JSON.stringify(item))
			},
			refresherrefresh() {
			...
			getFindParentDeviceTypeList() {
				...
				findDeviceTypeListFireproof(this.dataForm).then(res => {
