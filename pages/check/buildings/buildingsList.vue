			...
			<uni-swipe-action>
				...
				<uni-swipe-action-item v-for="(item,index) in buildingList" :key="index" :threshold="50">
					<view class="faultView" @click="goPage(item)">
						...
						<view class="contentLi">
							<view class="contentLi1">地上层数：{{item.upstairsFloor}}</view>
							<view class="contentLi2">地下层数：{{item.undergroundFloor}}</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			goPage(item) {
				this.$tab.navigateTo("/pages/check/buildings/addBuilding?item=" + JSON.stringify(item))
			},
			onreachBottom(e) {
			...
			getFindBybuildingList(empty) {
				...
				buildingList(this.dataForm).then(res => {
