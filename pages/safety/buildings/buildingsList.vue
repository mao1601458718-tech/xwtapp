				...
				<uni-swipe-action-item v-for="(item,index) in buildingList" :key="index" :threshold="50">
					<view class="faultView" @click="goPage(item)">
						...
						<view class="contentLi">
							<view class="contentLi1">占地面积：{{item.acreageDesc}}㎡</view>
							<view class="contentLi2">建筑面积：{{item.buildingAcreageDesc}}㎡</view>
						</view>
						<view class="contentLi">
...
<script>
	...
	export default {
		...
		methods: {
			goPage(item) {
				this.$tab.navigateTo("/pages/safety/buildings/addBuilding?item=" + JSON.stringify(item))
			},
			onreachBottom(e) {
			...
			getFindBybuildingList(empty) {
				...
				findByProjectList(this.dataForm).then(res => {
