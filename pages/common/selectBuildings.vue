...
<script>
	...
	export default {
		...
		methods: {
			getFindBuildingFloor() {
				let data = {
					"projectId": this.$store.state.user.projectId,
					"checkProjectId": this.$store.state.user.checkProject.id
				}
				this.selectBuildingFloor = {};
				this.buildingFloor = [];
				if (this.$store.state.user.atType == "maintenance") {
					findBuildingFloor(data).then(res => {
						this.buildingFloor = res.listObj
						this.buildingFloor.forEach((item) => {
				...
				} else if (this.$store.state.user.atType == "project") {
					projectFindBuildingFloor(data).then(res => {
						this.buildingFloor = res.listObj
						this.buildingFloor.forEach((item) => {
							if (this.selectBuildingFloor[item.buildingId]) {
								this.selectBuildingFloor[item.buildingId].push(item);
			...
			getFindBybuilding() {
				...
				if (this.$store.state.user.atType == "maintenance") {
					findBybuilding(data).then(res => {
						this.buildingDataList = res.object.list;
						this.getFindBuildingFloor();
