	...
	<view class="paveView">
		...
		<scroll-view scroll-y refresher-background="#fff" class="liListView" :refresher-threshold="105"
			...
			<view class="litView" v-for="(item,index) in planList" :key="index" @click="goFacilityList(item)">
				<view class="liTitle">{{item.customerName}}--{{item.planName}}</view>
				<view class="liRight">
					<view class="liText">{{item.maintenanceProjectName}}</view>
					<view class="liText">{{item.reportCode}}</view>
	...
	export default {
		...
		methods: {
			...
			getFindPlanManagementReportList(empty) {
				...
				findPlanManagementReportList(this.dataForm).then(res => {
					if (res.listObj) {
			...
			goFacilityList(item) {
				let url = item.reportFileUrl || item.temReportFileUrl;
				if (!url) {
