	...
	export default {
		...
		onLoad(data) {
			if (data.item) {
				...
				console.log(this.TypeForm.checkImgUrls)
				if (this.TypeForm.checkImgUrls && this.TypeForm.checkImgUrls != ";" && this.TypeForm.checkImgUrls !=
					"isNull") {
					let imgs = this.TypeForm.checkImgUrls.split(";");
					imgs.forEach((item) => {
						this.imgList.push({
							src: item,
							text: "上传成功",
		...
		methods: {
			...
			removeImg(data) {
				...
				if (data.length == 0) {
					...
					this.TypeForm.checkImgUrls = list.join(';');
				}
			},
			uploadSucceed(data) {
				if (data.length == 0) {
