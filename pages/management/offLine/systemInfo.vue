<template>
	<view class="container">
		...
		<!-- 设备列表 -->
		<view class="list">
			<block v-for="(item, index) in typeForm.planCheckItemReportList" :key="index">
		...
		<view class="bottomView">
			<view class="viewLeft" @click="postSaveCheckRecord()">离线保存</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		onLoad(data) {
			...
			uni.$on('pageData', (data) => {
				if (data.typeForm) {
					const index = this.typeForm.planCheckItemReportList.findIndex(item => item
						.deviceCheckItemId === data.typeForm
						.deviceCheckItemId);
					if (index !== -1) {
						this.typeForm.planCheckItemReportList.splice(index, 1, data.typeForm);
					}
					this.radioKey++;
				}
			});
		...
		methods: {
			oneKey() {
				this.typeForm.planCheckItemReportList.forEach((item, index) => {
					item.checkResult = 1
				})
				this.radioKey++;
			},
			groupChange(value, item) {
				if (value == 2) {
					this.$tab.navigateTo("/pages/management/offLine/systemTabInfo?item=" + JSON.stringify(item))
				}
			},
			goPage(item) {
			...
			postSaveCheckRecord() {
				...
				this.typeForm.planCheckItemReportList.forEach((item) => {

					if (!item.checkResult) {
						allCheckResult = false;
					} else {
						item["checkType"] = 1
						item["deviceCheckItemId"] = item.checkItemId;
						item.checkNum = item.totalCheckNum;
					}
				})
				if (!allCheckResult) {
					uni.$u.toast('还有未完成项目')
				...
				if (getStorage && getStorage.planList) {
					...
					getStorage.planList.forEach((item) => {
						if (item.id == this.typeForm.id) {

							item.planDeviceSysReportList.forEach((sys) => {
								if (sys.deviceSysId == this.typeForm.deviceSysId) {
									sys.checkNum = 0;
									sys.planDeviceTypeReportList.forEach((type) => {
										if (type.deviceTypeId == this.typeForm.deviceTypeId && type
											.deviceSysId == this.typeForm.deviceSysId) {

											type.checkNum = this.typeForm.totalCheckNum;
											type.planCheckItemReportList = this.typeForm
												.planCheckItemReportList;
										}
										sys.checkNum = sys.checkNum + type.checkNum;
									})
