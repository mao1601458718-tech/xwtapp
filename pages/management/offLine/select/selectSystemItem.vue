<template>
	<view class="pageDiv" scroll-y="true">
		...
		<view class="systemDiv" v-if="deviceTypeList.length>0" v-for="(item,index) in deviceTypeList " :key="index"
		...
		<u-empty v-if="!deviceTypeList||deviceTypeList.length==0" mode="list" icon="/static/images/nodata.png">
		</u-empty>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getFindParentDeviceTypeList() {
				...
				if (getStorage && getStorage.deviceType) {
					getStorage.deviceType.forEach((item) => {
